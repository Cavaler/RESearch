...
void DoEditReplace(int FirstLine, int StartPos, int &LastLine, int &EndPos, const tstring &Replace)
{
	...
	if (LastLine == FirstLine) {
		...
		bEmptyLastLine    = strGetString2.empty();
	}
	...
	do {
		...
		if (bLastOne) {
			if (bEmptyLastLine) {
				//	Simulate setting last empty line
				Position.CurLine++;
...
bool CERPresetCollection::EditPreset(CPreset *pPreset)
{
	...
	Dialog.Add(new CFarCheckBoxItem(35,11, 0, MInSelection, &pPreset->m_mapInts["InSelection"]));
